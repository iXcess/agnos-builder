...
def test_setup():
  ...
  proc = subprocess.Popen('/usr/kommu/setup', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
  time.sleep(5)
  assert proc.poll() is None
  proc.terminate()

def test_reset():
  proc = subprocess.Popen('/usr/kommu/reset', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
  time.sleep(5)
  assert proc.poll() is None
  proc.terminate()
